...
class EKF_raw_no_vel(EKF_proto_c):
    """
    class containign all the logic for Extended Kalman Filter calculation, does not contain velocity of the
    """
    ...
    def __init__(self, mode : str, anchors : np.ndarray, q_std = 0.1, r_std=0.1):
        ...
        self.x_ = np.zeros((3, 1), dtype=np.float32)        # state vector

        self.Q_ = np.eye(len(self.x_)) * (q_std**2)                    # process noise covariance matrix
        self.R_ = np.eye(self.n_) * (r_std**2)              # sensor noise covariance matrix

        self.P_ = np.eye(len(self.x_)) * 1.0                           # prediction covariance
        
        self.F_ = np.eye(len(self.x_))
    ...
    def update_sync_(self, distances : np.ndarray):
        ...
        # H: Jacobian matrix
        H = np.zeros((self.n_, len(self.x_)))
        ...
        self.P_ = np.dot((np.eye(len(self.x_)) - np.dot(K, H)), self.P_)
...
class EKF_raw_w_vel(EKF_proto_c):
    """
    class containign all the logic for Extended Kalman Filter calculation contains velocity of the robot
    """
