from abc import ABC, abstractmethod
import numpy as np
from custom_msgs.msg import Distances
from builtin_interfaces.msg import Time

from rclpy import logging



from localisation_engine.EKF_proto import EKF_proto_c

class EKF_w_callibration(EKF_proto_c):
    
    
    def __init__(self, mode : str, anchors : np.ndarray,  q_robot_stdev = 0.1, q_anchor_stdev = 0.1,r_std=0.1):

        ############################################## Atributes ##############################################
        
        self.anchors_ = anchors                                                             # anchor coords
        self.n_ = anchors.shape[0]                                                          # number of anchors
        self.x_ = np.zeros((6 + 3 * self.n_, 1), dtype=np.float32)               # state vector

        self.Q_ : np.matrix = np.zeros((6 + 3* self.n_,6 + 3* self.n_))                                        # because there are 2 different noise stdev matrix is initialised to zero
        
        
        # process noise covariance matrix
        self.R_ = np.eye(self.n_) * (r_std**2)                                              # sensor noise covariance matrix

        self.P_ = np.eye(6 + 3 * self.n_) * 1.0                                                           # prediction covariance
        
        self.last_timestamp_ = Time()
        self.last_timestamp_.sec = 0
        self.last_timestamp_.nanosec = 0
         
    

        self.get_new_state_imp_ : function = None

        if(mode == "SYNC"):
            self.get_new_state_imp_ = self.get_new_state_sync_
        elif(mode == "ASYNC"):
            self.get_new_state_imp_ = self.get_new_state_async_


        
        self.Q_[0,0] = self.Q_[1,1] = self.Q_[2,2] = q_robot_stdev ** 2
        
        for i in range(6, self.Q_.shape[0]):
            self.Q_[i, i] = q_anchor_stdev ** 2
            
                
        for i in range(self.n_):
            self.x_[6 + 3*i + 0, 0] = anchors[i, 0]
            self.x_[6 + 3*i + 1, 0] = anchors[i, 1]
            self.x_[6 + 3*i + 2, 0] = anchors[i, 2]
            
    
    @DeprecationWarning
    def get_new_state_sync_(self, msg : Distances):
        logging.get_logger().error("Method not implemented")
        pass

    def get_new_state_async_(self, msg : Distances):
        
        
        dt = Time()
        dt.sec = msg.stamp.sec - self.last_timestamp_.sec
        dt.nanosec = msg.stamp.nanosec - self.last_timestamp_.nanosec
        dt_float = dt.sec + dt.nanosec * 1e-9

        self.predict_async_(dt=dt_float)
        self.update_async_(msg=msg)
        
        # update the value for last_timestamp_
        self.last_timestamp_ = msg.stamp


        
        return self.x_
        
        
        
        
    
    @DeprecationWarning
    def predict_sync_(self, dt : float):
        logging.get_logger().error("Method not implemented")
        
    
    
    def predict_async_(self, dt : float):
        
        F = np.eye(N=6 + 3 * self.n_, dtype=np.float32) # transition matrix
        F[0, 3] = F[1, 4] = F[2, 5] = dt

        # apriori x prediction

        # x = F * x
        self.x_ = np.dot(F, self.x_)
        
        # P = F * P * F^T + Q
        self.P_ = np.dot(np.dot(F, self.P_), F.T) + self.Q_
        
        
        
    
    @DeprecationWarning
    def update_sync_(self, msg  : Distances):
        logging.get_logger().error("Method not implemented")
        
    
    
    def update_async_(self, msg  : Distances):
        
        
        # measurements is a list/array of distances to each anchor
        z = np.array(msg.distances).reshape(-1, 1)

        mra = msg.mra # most recent anchor
        
        # h(x): Predicted distances based on current state
        hx = 0
        # H: Jacobian matrix3
        H = np.zeros((self.n_, 6 + 3 * self.n_))
        
        anchor = self.x_[6 + 3*mra : 6 + 3*mra + 3].flatten()  

        dx = self.x_[0, 0] - anchor[0]
        dy = self.x_[1, 0] - anchor[1]
        dz = self.x_[2, 0] - anchor[2]


        dist = np.sqrt(dx**2 + dy**2 + dz**2)
        
        # Avoid division by zero
        if dist < 1e-6: dist = 1e-6
        
        hx = dist
        
        # Partial derivatives for position
        H[mra, 0] = dx / dist
        H[mra, 1] = dy / dist
        H[mra, 2] = dz / dist
        # Derivatives for velocity are 0
        H[mra, 3] = H[mra, 4] = H[mra, 5] = 0
        
        
        

        # partial derivatives for anchor position
        # the value is the same, sign(+/-) is changed
        
        H[mra, 6 + 3*mra + 0] = - dx / dist
        H[mra, 6 + 3*mra + 1] = - dy / dist
        H[mra, 6 + 3*mra + 2] = - dz / dist



        # EKF Update Equations
        e = z - hx  # Innovation (Error)
        S = np.dot(np.dot(H, self.P_), H.T) + self.R_ # Innovation Covariance
        K = np.dot(np.dot(self.P_, H.T), np.linalg.inv(S)) # Kalman Gain
        
        self.x_ = self.x_ + np.dot(K, e)
        self.P_ = np.dot((np.eye(6 + 3 * self.n_) - np.dot(K, H)), self.P_)
        
        
    def get_new_state(self, distances):
        return self.get_new_state_imp_(distances)